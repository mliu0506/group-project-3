...
class Rentals extends Component {
	...
	getDays = date => {
		//date is the array that is passed from the calendar when days are selected.
		let temp = [];
		let range = [];
		...
		for (let i = 0; i < days; i++) {
			//adds each day (including last) to range array
	...
	checkAvailability = itemRes => {
		//passed all the reservations for a given item
		for (let i = 0; i < itemRes.length; i++) {
			//iterate through all individual reservations to compare to selected dates one at a time
			...
			for (let k = 0; k < this.state.unix.length; k++) {
				//compares each index in this.state.unix to each index in range
			...
			<Fragment>
				...
				<div className="main-container">
					...
					<div className="calendar-container">
						<h1 className="calendar-head-title"></h1>
						<Calendar
